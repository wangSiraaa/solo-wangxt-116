...
- 每个方案带可验证事件账本：事件含唯一 ID、父事件哈希、逻辑时钟、XML 摘要、路径快照、方案版本与状态前后摘要，形成哈希链；
- IndexedDB 同时保存账本、检查点和迁移信息；刷新后从最后可信检查点确定性恢复光标、节拍、循环边界、队列进度和完成历史；
- 链缺失、哈希损坏、路径身份失配或不可达事件会显式阻塞播放并进入可见隔离区，可选择拒绝、保留历史或分叉为只读审计线，绝不按书面小节猜测；
- 导出增加账本文件（XML、标记、方案、账本四个文件）；账本导入按事件 ID 幂等，乱序事件按父链因果折叠，同 ID 不同内容/父链冲突/摘要不符进入隔离区，不能覆盖本地较新状态；
- 队列播放、暂停/恢复、边界完成、失配重绑、提案合并/撤销均追加不可变事件；
...
- 导出四个文件：
  ...
  - `*.rehearsal-ledger.json`：哈希链事件账本、检查点和隔离记录，导入前校验 XML 摘要。
